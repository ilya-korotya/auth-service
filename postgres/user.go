...
var (
	...
	insertUserToken = "INSERT INTO tokens (token, user_id, expired_at) VALUES ($1, $2, $3) RETURNING id, token, user_id, expired_at"
)
...
func (u *UserStore) Set(user *model.User) (*model.User, error) {
	...
	if err := tx.QueryRow(insertUserToken, user.Token.Content, user.ID, user.Token.ExpiredAt).Scan(
		&user.Token.ID,
		&user.Token.Content,
		&user.Token.UserID,
